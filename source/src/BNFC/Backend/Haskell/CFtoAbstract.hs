...
cf2Abstract :: Bool     -- ^ Use ByteString instead of String
            ...
            -> CF       -- ^ Grammar
            -> String
cf2Abstract byteStrings ghcExtensions functor name cf = render $ vcat $
  (if ghcExtensions then "{-# LANGUAGE DeriveDataTypeable #-}" else empty) :
  (if ghcExtensions then "{-# LANGUAGE DeriveGeneric #-}" else empty) :
  ("module" <+> text name <+>  "where\n") :
...
prSpecialData byteStrings position classes cat =
    ...
    $$ "instance IsString" <+> ppcat <+> "where fromString =" <+> ppcat
  where
    ppcat = text (show cat)
